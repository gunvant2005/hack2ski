  ...
  return (
    <header className="sticky top-0 z-50 bg-white/85 backdrop-blur-md border-b border-slate-200/80 text-slate-900 shadow-subtle transition-minimal">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        {/* Brand Logo */}
        <Link href={user ? "/dashboard" : "/"} aria-label="LegalLens AI - Home" className="flex items-center gap-2.5 font-bold text-lg tracking-tight text-slate-900 hover:opacity-90 transition-opacity">
          <div className="w-8 h-8 rounded-lg bg-slate-900 text-white flex items-center justify-center shadow-sm" aria-hidden="true">
        ...
        <nav className="hidden md:flex items-center gap-1" aria-label="Main navigation">
          {user ? (
            <>
              ...
              >
                <Layers className="w-3.5 h-3.5 text-slate-700" aria-hidden="true" />
                Compare Documents
              </Link>
            </>
          ) : (
      ...
      {mobileMenuOpen && (
        <div id="mobile-navigation" className="md:hidden bg-white border-t border-slate-100 shadow-lg animate-in fade-in slide-in-from-top-2" role="navigation" aria-label="Mobile navigation">
          <div className="max-w-7xl mx-auto px-4 py-4 space-y-2">
            {user ? (
              <>
                {/* User info */}
                <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-200/80 mb-3">
                  <div className="w-9 h-9 rounded-full bg-slate-900 text-white font-bold flex items-center justify-center text-sm shadow-sm" aria-hidden="true">
                    {user.name ? user.name.charAt(0).toUpperCase() : 'U'}
                  </div>
                  <div>
                ...
                >
                  <Layers className="w-4 h-4 text-slate-700" aria-hidden="true" />
                  Compare Documents
                </Link>
